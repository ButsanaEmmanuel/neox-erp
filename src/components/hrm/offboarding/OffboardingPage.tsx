// HRM-2.2 — Offboarding page on the new DB-driven API.
// Mirror of the refactored OnboardingPage. Self-service vs admin
// scoping based on usePermissions().has('hrm.offboarding.execute').

import React, { useCallback, useEffect, useState } from 'react';
import { CheckCircle, Clock, Circle, User, Monitor, Shield, Briefcase, CheckCircle2, LogOut, RefreshCw, Plus } from 'lucide-react';
...
import TemplateManagerModal from '../onboarding/TemplateManagerModal';
import {
  offboardingApi,
...
const OffboardingPage: React.FC = () => {
  ...
  const canExecute = isReady && has('hrm.offboarding.execute');
  const canManageTemplates = isReady && has('hrm.offboarding.write');
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const r = await offboardingApi.listChecklists(scope === 'self' ? { forUserId: actorUserId } : {});
      setChecklists(r.checklists);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to load offboarding checklists');
    } finally {
      setLoading(false);
    }
  }, [actorUserId, scope]);
  ...
  useEffect(() => {
    ...
    offboardingApi.getChecklist(selectedId)
  ...
  const handleCycleTask = async (task: ChecklistTaskRow) => {
    ...
    try {
      await offboardingApi.updateTask(detail.id, task.id, { statusCode: next }, actorUserId);
      const r = await offboardingApi.getChecklist(detail.id);
  ...
  const handleCompleteChecklist = async () => {
    if (!detail) return;
    try {
      await offboardingApi.completeChecklist(detail.id, actorUserId);
      notify('👋 Offboarding complete', 'success');
      setSelectedId(null);
      await reload();
    } catch (e) {
  ...
  if (selectedId && detail) {
    ...
    for (const t of detail.tasks) grouped[ownerOf(t.templateTask.assignedRole)].push(t);
    ...
    return (
      <div className="h-full flex flex-col overflow-hidden bg-app">
        <PageHeader
          title={`Offboarding: ${detail.user.name ?? detail.user.email ?? detail.userId}`}
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-4">
        {loading && !checklists ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-[15px] font-semibold text-muted">No offboarding checklist</p>
