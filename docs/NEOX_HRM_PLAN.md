...
| DH10 | 🟡 Data sécurité (ouverte, priorité moyenne — découverte sprint Dettes D6 Commit B) | Le rôle super_admin (seeded) ne grant que **98 / 108 permissions** du registry. Notamment manquantes : `finance.entries.read/approve/reject/create/update/settle/submit_validation`, `finance.evidence.upload`, `finance.reconciliation.resolve`, `finance.settings.manage`. Bouché manuellement pour `ebutsana@neox.io` via `scripts/grant-admin-full-perms.mjs` (idempotent, exécuté). **Reste à faire** : (1) corriger le seed `prisma/seed/rbac.seed.mjs` pour qu'un nouveau super_admin reçoive automatiquement les 108 permissions ; (2) appliquer le script à tous les autres comptes ADMIN existants. | Sprint RBAC dédié |
| DH11 | 🟡 Data (ouverte, priorité basse — découverte sprint Dettes D6 Commit A) | Le `Permission` registry contient `pm.milestones.read/write/execute` mais **pas** `pm.milestones.delete`. La route `DELETE /api/v1/projects/:id/milestones/:mId` a donc été gatée sur `pm.milestones.write` (workaround documenté inline). Asymétrie avec `pm.projects.delete`, `pm.workItems.delete`, `pm.documents.delete` qui existent. | Sprint RBAC dédié — ajouter `pm.milestones.delete` au registry (seed Permission row + migration UserPermissionSet pour les ADMINs existants) puis swap la gate sur la route. |
