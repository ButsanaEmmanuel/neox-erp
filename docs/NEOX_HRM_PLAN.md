...
| DH7 | Colonnes configurables par utilisateur — vue work items télécom. Le tableau télécom a 35 colonnes. Chaque utilisateur doit pouvoir choisir et réordonner les colonnes à afficher. Modèle `UserColumnPreference (userId, context, columns Json)`. UI : panneau colonnes dans la toolbar + persisté en DB. | Sprint dettes |
