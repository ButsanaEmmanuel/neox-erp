...
| DH2 | ✅ Résolue 2026-05-24 (Sprint Dettes Techniques) | Modèle `PublicHoliday` (`(date, country)` unique, `isActive` flag, country default `'CD'`) + migration `20260524084959_add_public_holiday` (premier `migrate dev` post-DH6, baseline a tenu). Seed `prisma/seed/publicHolidays.seed.mjs` idempotent — 24 entrées RDC pour 2025+2026 (9 fixes/an + 3 mobiles/an : Vendredi Saint, Lundi de Pâques, Ascension — dates hardcodées). API `calculateLeaveDays(start, end, holidaysSet?)` pure (Set optionnel) + async wrapper `calculateLeaveDaysWithHolidays(prisma, start, end, country='CD')` câblé dans `createRequest`. Garde anti-double-décompte (weekend court-circuite la vérif holiday). Tests `hrm-leave.test.mjs` : 11/11 ✓ (6 legacy + 4 nouvelles DH2 + intégration). 3 commits : `db2a0a1`, `98e2d55`, `18623e2`. |
...
| DH6 | ✅ Résolue 2026-05-24 (Sprint Dettes Techniques) | **Résolu** : 20 migrations historiques (mars→mai 2026) collapse en un baseline unique `00000000000000_baseline` (2806 L SQL) généré via `prisma migrate diff --from-empty --to-url $DATABASE_URL --script`. Cause racine confirmée : `Project`/`WorkItem` créés via `prisma db push` sans migration init → toute migration ultérieure faisant `ALTER TABLE "Project"` cassait le shadow DB. Historique préservé dans `prisma/migrations_archive/` avec README. `_prisma_migrations` nettoyé (20 rows historiques deleted, baseline marqué applied via `migrate resolve`). Vérifié : `prisma migrate dev --create-only` produit une migration vide = shadow DB match exact `schema.prisma`. **`prisma migrate dev` débloqué pour DH2, DH7, D13.** Hash `71491a7`. |
| DH7 | ✅ Résolue 2026-05-24 (Sprint Dettes Techniques) | Modèle `UserColumnPreference` (userId+context unique, columns JSON, relation User) — migration `20260524095948_add_user_column_preference`. Service backend `columnPreferences.service.mjs` avec registry `CONTEXT_REGISTRY` (premier context : `telecom_workitems` — 35 colonnes autorisées + 9 défauts ex-Excel). 2 routes `GET/PUT /api/v1/preferences/columns` (gated `pm.workItems.read`/`write`), validation complète (UNKNOWN_CONTEXT, UNKNOWN_COLUMN_KEY, EMPTY_COLUMNS, USER_ID_REQUIRED, dedupe silencieux, filter de stale keys). 8 assertions de test `column-preferences-dh7.test.mjs` ✅. Frontend : `src/services/pm/columnPreferences.service.ts`, `src/components/pm/telecom/telecomColumns.ts` (registry 35 colonnes avec accessors typés), `src/components/pm/telecom/ColumnConfigPanel.tsx` (slide-in + checkbox + dnd-kit sortable — pas de nouvelle dep, déjà présent — + Reset + Apply), bouton ⚙ "Columns" dans toolbar `WorkItemsPage` (télécom only), rendu dynamique `<th>`/`<td>` télécom. Status + Finance Sync gardés hardcodés (affordances UI spécifiques). Première colonne "Identifier" non-configurable (porte checkbox + expand sub-tasks D13 + badge X/Y). `tsc --noEmit` ✅. 3 commits : `66a16ca`, `7f8288f`, `cf4975b`. |
| DH8 | ✅ Résolue 2026-05-24 (Sprint Dettes Techniques) | Grep `WeekHeader\|WeekEditor\|WeekSummary` sur `src/` → uniquement self-references (WeekSummary importe WeekEditor ; WeekHeader était isolé depuis sa migration `can()`→`usePermissions` en D6 commit `93124fd`). Aucun consumer extérieur. Les 3 fichiers supprimés. `tsc --noEmit` ✅. Commit `8a1b554`. |
| DH9 | ✅ Résolue 2026-05-25 (Sprint DH9 — Audit RBAC cross-modules) | Avant ce sprint, **17 routes Finance acceptaient des appels anonymes** (gated en D6 Commit B `138378a`) + reste de l'audit non couvert. | **Résolu** : audit complet `auth-server.mjs` (2154 L), 70 routes gardées ou migrées sur 5 commits (1 par étape). **Étape 1** (`3bca522`) registry étendu 109→134 keys (CRM +5, system DH9 +7, finance fine-grained +13) ; `system.dashboard.read` + `system.notifications.{read,write}` propagés à tous les rôles métier (option (a) — pas de magie). **Étape 2** (`db56873`) 10 routes CRM (3 migrations `assertModuleAccess` + 7 nouvelles gates). **Étape 3A** (`c83e645`) 17 routes Finance CRUD (invoices/bills/payments/receipts + entries/:id family + receivables/payables détail). **Étape 3B** (`875803c`) 23 routes Finance non-CRUD (snapshot/reports + 7 settings + 3 backfills + 4 SCM finance bridges + 7 reconciliation — `/finance/scm/*` étaient en fait ungated contrairement à l'hypothèse initiale). **Étape 4** (`337334d`) 20 routes misc (Dashboard, Reports intelligence, Notifications, Access admin, Projects repair-integrity, 5 migrations HRM legacy, PM details/files/activities). **Étape 5** (`cc5a770`) `docs/RBAC_GUIDELINES.md` créé — pattern standard, convention nommage, règle seed-avant-route, règle couverture rôles, tableau modules/keys, instructions HSE+SCM, statut `assertModuleAccess` legacy, exemple complet. Plus aucun `assertModuleAccess` inline dans `auth-server.mjs`. `node --check` ✓ après chaque commit. Décision préalable Étape 0 : tableau de couverture validé avant tout code, 5 points critiques tranchés explicitement (Dashboard/Notifs option (a), CRM 10 routes, SCM finance non gardées, hrm.bootstrap = .read, discrepancies = nouvelle `.write`). |
| DH10 | ✅ Résolue 2026-05-24 (Sprint RBAC+D5 — DR2) | Le rôle super_admin (seeded) ne grant que **98 / 108 permissions** du registry. Notamment manquantes : `finance.entries.read/approve/reject/create/update/settle/submit_validation`, `finance.evidence.upload`, `finance.reconciliation.resolve`, `finance.settings.manage`. Bouché manuellement pour `ebutsana@neox.io` via `scripts/grant-admin-full-perms.mjs` (idempotent, exécuté). | **Résolu** : (1) Les 10 permissions legacy ajoutées au bloc PERMISSIONS de `prisma/seed/rbac.seed.mjs` sous une section "Finance — legacy registry (pre-HRM-1.1)" documentée. Comme `super_admin` utilise `ALL_KEYS` et `finance_admin` spread `...FIN_KEYS`, les deux rôles couvrent désormais les 10 keys via `RolePermission` — plus de bandaid `UserPermissionSet`. (2) Seed ré-exécuté : super_admin 99→**109** (+10), finance_admin 16→26 (+10), readonly 35→36 (+1 via `finance.entries.read`). (3) Idempotence vérifiée : 2e run = 0 new permission / 0 new link. (4) Test e2e : nouveau user créé + assigné `super_admin` via UserRole → `getUserPermissions` retourne **109/109** sans aucune entrée `UserPermissionSet`. (5) Script `scripts/grant-admin-full-perms.mjs` **supprimé** (devenu redondant ; le seed est désormais la source unique de vérité). Les entrées `UserPermissionSet` créées par le script pour `ebutsana@neox.io` restent en place (shadowing inoffensif ; cleanup non requis pour fermer DH10). |
...
| DH12 | ✅ Résolue 2026-05-25 (Sprint DH12 — Milestone parentId / hiérarchie sous-milestones) | Sous-tâches `Milestone.parentId` non implémentées avant DH12. | **Résolu** : 4 commits sur `claude/sprint-dh12`. **C1** (`0598027`) migration `20260525120000_add_milestone_parent` — `parentId String?` + self-relation `MilestoneChildren` (`onDelete: Restrict`) + `@@index([parentId])`, appliquée via `prisma migrate deploy`. **C2** (`2fa8936`) `backend/services/pm/milestoneHierarchy.service.mjs` calqué sur `workItemHierarchy.service.mjs` (4 fonctions : `createSubMilestone` / `listSubMilestones` / `moveSubMilestone` / `rollupStatus`). Décision (B) : pas de statut `at_risk` — rollup contraint à la whitelist Milestone `{planned, in_progress, done, blocked}` (≥1 enfant blocked → parent blocked). Pas de rollup auto sur `completionPct` (édition manuelle conservée). 3 routes (`assertPermission`) : POST/GET `/api/v1/pm/milestones/:id/sub-milestones`, PATCH `/api/v1/pm/milestones/:id/parent`. **C3** (`90ac71e`) UI récursive : type étendu (`parentId?` + `children?`), API client +3 endpoints, store `createSubMilestone` + refetch tree, `MilestoneNode` récursif (depth-based marginLeft), `MilestoneCard` étendue (chevron, badge X/Y done, warning child blocked, bouton `+` désactivé+tooltip à depth ≥ MAX_DEPTH-1), `MilestoneFormModal` mode `'create-sub'`. `tsc --noEmit` ✓. **C4** (`7a40c77`) `backend/tests/pm/pm-milestones-hierarchy.test.mjs` — **10/10 ✓** au premier run (parentId setté, depth>3, rollup done/blocked/récursif, cycle, cross-project, FK Restrict P2003, soft-delete child rollup, listSubMilestones nested). D13 désormais totalement fermée (WorkItem + Milestone). |
