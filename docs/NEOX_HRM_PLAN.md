...
| DH9 | 🔴 Sécurité (ouverte, **priorité haute** — découverte sprint Dettes D6 Commit B) | Avant ce sprint, **17 routes Finance acceptaient des appels anonymes** : `/finance/hrm/expense-claims` GET/POST/approve, `/finance/hrm/employee-advances` GET/POST/approve, toutes les routes `/finance/hrm/payroll-*` (schedules/runs/batches/lines GET/POST/PATCH/post/approve/reconcile/disburse/adjust). D6 Commit B (`138378a`) a gates avec `assertPermission` sur les 17 — fix d'urgence. **Reste à faire** : audit sécurité complet de **tous** les autres endpoints `/api/v1/*` pour identifier les routes encore non-gardées (CRM, SCM, HSE, system…). Établir une liste exhaustive et soit `assertPermission`, soit décision explicite "endpoint public". | Sprint RBAC dédié (priorité 5 roadmap) — **avancer avant les modules Finance/Reporting** |
