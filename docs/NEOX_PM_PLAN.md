...
**Statut : ✅ Complet — 7/7 (100%). Dettes soldées : D1, D3, D4, D9, D11, D12, D14. Restent ouvertes : D2, D5, D6, D7, D8, D13, D15.**
...
- [x] `tsc --noEmit` zéro erreur
### Tâche 7.6 — D14 structured 409 blockers[]
- [x] Service `milestones.service.mjs` — `err.blockers = [{ id, title }]` attaché avant `throw`
- [x] Route handler `projects.routes.mjs` — forward `err.blockers` et `err.field` dans le payload JSON
- [x] Frontend `MilestonesPage.tsx` — extraction depuis `ApiError.payload.blockers` quand `code === 'MILESTONE_BLOCKED'`
- [x] `DeleteMilestoneModal` — rendu liste structurée (`<ul>` + icône Link2), plus de message-sniffing
- [x] Smoke E2E : `POST A → POST B dep A → DELETE A` → 409 + `blockers: [{ id, title }]` ✓
- [x] `tsc --noEmit` + `node --check` zéro erreur
### Tâche 7.7 — D1 formal closure
- [x] Grep exhaustif des 4 SSE annoncés Sprint 1.3/1.4 → 4/4 présents dans `routes/pm/projects.routes.mjs` post-Sprint 6.1
- [x] Architecture parallèle `/routes/pm/` confirmée comme décision design (Sprint 2 Phase 2), pas une dette
- [x] D1 marquée résolue, pas de D1b nécessaire
...
| 2026-05-22 | 7 | 7.7 — D1 formal closure | ✅ Complet | Grep exhaustif des 4 SSE annoncés Sprint 1.3/1.4 dans le code post-Sprint 6.1 : `project_scope_updated` (l.181), `work_item_created` (l.114), `work_item_updated` (l.125 + `projectItemDetails:571` pour le path `/details`), `work_item_deleted` (l.135) — tous dans `routes/pm/projects.routes.mjs`. Décision de réécriture from-scratch (vs cherry-pick) actée dans `NEOX_PM_SSE_AUDIT.md` §6 et appliquée en Sprint 6.1. L'architecture parallèle `/routes/pm/` est une décision Sprint 2 Phase 2 figée, pas une dette. **D1 fermée**, pas de D1b. **Sprint 7 = 7/7 (100%).** Total dettes soldées Sprint 7 : D1, D3, D4, D9, D11, D12, D14. Dettes ouvertes restantes : D2, D5, D6, D7, D8, D13, D15. |
| 2026-05-22 | 7 | 7.6 — D14 structured 409 blockers[] | ✅ Complet | 3 fichiers. Service : `blockersList = [{ id, title }]` attaché via `err.blockers` avant `throw conflict()`. Route handler : forward `err.blockers` + `err.field` dans le payload JSON (`projects.routes.mjs:236-239`). Frontend : nouveau state `deleteBlockers`, extraction depuis `ApiError.payload.blockers` si `code === 'MILESTONE_BLOCKED'`, prop dédiée au modal. `DeleteMilestoneModal` : `isBlocked = (blockers?.length ?? 0) > 0` (plus de message-sniffing), rendu `<ul>` avec icône `Link2` par dépendant. Smoke E2E confirmé : `POST A → POST B dep A → DELETE A` → 409 + payload `{ error, code, blockers: [{id,title}] }`. `tsc --noEmit` ✅ + `node --check` ✅. **D14 fermée**. Sprint 7 = 6/7. |
...
| 2026-05-24 | RBAC+D5 | Sprint clôture — DR1 DR2 DR3 D5 DR4 | ✅ Complet | **5 dettes fermées** sur branche `claude/sprint-rbac-d5`. DR1 (`5d0127f`) `pm.milestones.delete` ajouté au registry + route migrée. DR2 (`48e53ad`) 10 perms `finance.*` legacy intégrées au seed → super_admin 100% registry (109/109) via rôle, script one-shot supprimé. DR3 (`357c7e7`) suppression `engineeringTeamProjectCount` (string heuristic résiduel `loadUserContext`). D5 (`8dec313`) audit live DB confirme 11/11 FKs corrects sur 4 modèles in-scope — aucune migration nécessaire (baseline DH6 + D13 couvrent tout). DR4 (`a8ab029`) `NEOX_PM_PERMISSIONS.md` enrichi (registry 20 PM keys, grille rôles, Changelog). Le seed `prisma/seed/rbac.seed.mjs` est désormais source unique 100% registry ; le chemin RBAC `loadUserContext` est entièrement DB-only. |
| 2026-05-25 | DH12 | Milestone.parentId hierarchy | ✅ Complet | **DH12 fermée** sur branche `claude/sprint-dh12`. 4 commits + clôture. **Commit 1** (`0598027`) migration `20260525120000_add_milestone_parent` — `parentId String?` + self-relation `MilestoneChildren` (`onDelete: Restrict`) + `@@index([parentId])`. Appliquée via `prisma migrate deploy`. **Commit 2** (`2fa8936`) nouveau service `backend/services/pm/milestoneHierarchy.service.mjs` calqué sur `workItemHierarchy.service.mjs` (4 fonctions : `createSubMilestone` depth≤3 + parent-rollup, `listSubMilestones` 2-levels, `moveSubMilestone` anti-cycle + anti-cross-project + depth guard, `rollupStatus` itératif in-tx). Décision (B) : pas de statut `at_risk` — rollup mappe sur la whitelist Milestone existante `{planned, in_progress, done, blocked}` (≥1 enfant blocked → parent blocked). `listProjectMilestones` filtré `parentId: null` + `include children` 3 niveaux pour éviter la duplication des nested dans la liste plate. `updateMilestone`/`deleteMilestone` déclenchent rollup sur le parent. 3 nouvelles routes (assertPermission) : `POST/GET /api/v1/pm/milestones/:id/sub-milestones`, `PATCH /api/v1/pm/milestones/:id/parent`. **Commit 3** (`90ac71e`) UI récursive : type `Milestone` étendu (`parentId?` + `children?`), API client +3 endpoints, store `createSubMilestone` + refetch après mutate/delete (rebuild tree). `MilestonesPage.tsx` : nouveau `MilestoneNode` récursif (depth-based `marginLeft`), boucle racine sur `!parentId`, `MilestoneCard` étendue (chevron expand/collapse, badge X/Y done, warning `Child blocked`, bouton `+` désactivé+tooltip à depth ≥ MAX_DEPTH-1), `MilestoneFormModal` mode `'create-sub'` (titre dédié, dependencies masquées). `flattenTree()` pour header counts + pool deps. `tsc --noEmit` ✓. **Commit 4** (`7a40c77`) `backend/tests/pm/pm-milestones-hierarchy.test.mjs` **10/10 ✓** au premier run (10 cas : parentId setté, depth>3, rollup done/blocked/recursive, cycle, cross-project, FK Restrict P2003, soft-delete child rollup, listSubMilestones nested). Pattern d'isolation : 2 projets jetables `__dh12_test_*__` wipés leaves-first en `finally`. D13 désormais entièrement fermée (WorkItem + Milestone). |
| 2026-05-25 | DH9 | Audit RBAC cross-modules | ✅ Complet | **DH9 fermée** sur branche `claude/sprint-dh9`. 6 commits (1 par étape + clôture). **Étape 0** tableau de couverture rôles validé avant tout code, 5 points critiques tranchés (Dashboard/Notifs option (a), CRM 10 routes vs 6 annoncées, SCM finance non gardées, hrm.bootstrap = .read, discrepancies = nouvelle `.write`). **Étape 1** (`3bca522`) registry 109→**134 keys** : CRM (5), system DH9 (7 : dashboard/notifications/reports/access), finance fine-grained (13 : invoices/bills/payments/receipts/backfill/reconciliation/snapshot). `system.dashboard.read` + `system.notifications.{read,write}` propagés à tous les rôles métier. **Étape 2** (`db56873`) 10 routes CRM (3 migrations `assertModuleAccess`→`assertPermission` + 7 nouvelles). **Étape 3A** (`c83e645`) 17 routes Finance CRUD/entries/receivables-payables. **Étape 3B** (`875803c`) 23 routes Finance non-CRUD (snapshot/reports/settings×7/backfills×3/SCM finance bridges×4/reconciliation×7). **Étape 4** (`337334d`) 20 routes misc (Dashboard, Reports intelligence, Notifications, Access admin, repair-integrity, HRM legacy×5, PM details/files/activities×6). **Étape 5** (`cc5a770`) `docs/RBAC_GUIDELINES.md` créé (265 L, 9 sections — référence opérationnelle). **Total : 70 routes gardées ou migrées.** Plus aucun `assertModuleAccess` inline dans `auth-server.mjs` (helper conservé pour modules importateurs legacy). `node --check` ✓ après chaque commit. DH9 marqué ✅ aussi côté `NEOX_HRM_PLAN.md`. |
...
|----|---------|---------|---------------|
| D1 | ✅ Résolue 2026-05-22 (Sprint 7.7) | Journal Sprint 1.3 / 1.4 annonçait les SSE `project_scope_updated`, `work_item_created`, `work_item_updated`, `work_item_deleted` émis depuis les routes. Cherry-pick depuis `vigorous-napier-03a79d` (commit `f79217c`) avait porté les migrations mais pas le code routes Sprint 1 (architecture parallèle `backend/routes/pm/` vs `backend/routes/`). | **Résolu** (réécriture from scratch en Sprint 6.1, décision actée dans `NEOX_PM_SSE_AUDIT.md` §6) : les 4 émetteurs promis Sprint 1.3/1.4 sont aujourd'hui présents dans `backend/routes/pm/projects.routes.mjs` — `project_scope_updated` (l.181), `work_item_created` (l.114), `work_item_updated` (l.125 — coexiste avec le path `/details` qui émet aussi depuis le service partagé `projectItemDetails:571`), `work_item_deleted` (l.135). 4/4 vérifiés par grep. L'architecture parallèle `/routes/pm/` reste une décision Sprint 2 Phase 2 documentée (cf. table "Décisions Techniques Figées"), pas une dette. Pas de D1b nécessaire. |
...
| D5 | ✅ Résolue 2026-05-24 (Sprint RBAC+D5) | Audit FK initial (Sprint 1) avait identifié `ProjectMember_projectId_fkey = ON DELETE RESTRICT` comme bloquant pour le hard delete projet. La résolution a finalement retenu **RESTRICT** comme défaut sûr (HRM-1.0 commit `0d66980`, consolidé dans `00000000000000_baseline` par DH6) — la suppression d'un projet, d'un user, ou d'un milestone bloquant doit être un acte volontaire passant par la couche service (soft-delete `isDeleted=true`), pas un side-effect d'un DELETE SQL. | **Résolu** : audit live de la DB confirme 11/11 FKs corrects sur les 4 modèles existants (`ProjectDocument` n'existe pas) : `ProjectMember.{projectId,userId,departmentId} = RESTRICT`, `ProjectScope.projectId = RESTRICT`, `Milestone.projectId = RESTRICT`, `Milestone.ownerId = SET NULL` (owner optionnel), `MilestoneDependency.milestoneId = CASCADE` (enfant attaché), `MilestoneDependency.dependsOnId = RESTRICT`, `WorkItem.projectId = RESTRICT`, `WorkItem.parentId = RESTRICT` (D13), `WorkItem.importBatchId = SET NULL` (optionnel). Critères de sortie vérifiés e2e : (a) `prisma.user.delete()` sur user avec `ProjectMember` actif → P2003 ✓ ; (b) soft-delete via `isDeleted=true` continue de fonctionner ✓. Aucune migration nécessaire — le baseline DH6 + D13 couvrent déjà tout. |
| D6 | ✅ Résolue 2026-05-24 (Sprint Dettes Techniques, étapes A+B+C) | Constat principal : shim `can()` hardcodé dans `src/lib/rbac.ts` + routes PM/Finance encore sur `assertModuleAccess` legacy. Constat secondaire (Sprint 3) : `loadUserContext` l.112-147 (`engineeringTeamProjectCount`) gardait un OR département `contains 'ENG'`/`'Engineering'`. | **Résolu** : (A, `eae1241`) 18 routes PM (15 dans `routes/pm/projects.routes.mjs` + 3 inline auth-server) bascule de `assertModuleAccess` vers `assertPermission(ctx, key)` avec keys `pm.*`. (B, `138378a`) 4 routes Finance converties + 17 routes auparavant non-gardées (expense-claims/employee-advances/payroll-*) gates avec keys `finance.*` — fix sécurité collatéral. (C, `93124fd`) shim `can()`/`canAccess()` + tables hardcodées (MODULE_OWNER_DEPARTMENTS, OWNER/TIERS_PERMISSIONS, TIERS_RESOURCE_ALLOWLIST, ModuleName/Action/Resource types, roleMap inline) supprimés de `rbac.ts` (365→185 L). WeekHeader migré au passage. Tests `backend/tests/pm/pm-routes-rbac-d6.test.mjs` (10+3) et `backend/tests/finance/finance-routes-rbac-d6.test.mjs` (11+3) verts. `tsc --noEmit` ✅. **Gaps tracés** : (i) `pm.milestones.delete` absent du registry — DELETE milestone gated sur `pm.milestones.write` pour l'instant ; (ii) admin n'avait que 98/108 perms registry — comblé via `scripts/grant-admin-full-perms.mjs` idempotent ; (iii) résidu `loadUserContext` (heuristique `engineeringTeamProjectCount`) — **résolu 2026-05-24 (Sprint RBAC+D5 — DR3)** : champ et requête supprimés ; signal redondant avec `projectMembershipCount` (compteur déjà DB-only sans filtre de rôle/département). Reason `engineering_team_assignment` retirée. Doc `NEOX_PM_PERMISSIONS.md` synchronisée. Vérifié e2e : admin → `omni_admin`, non-admin membre → `project_manager_assignment`/`project_membership` selon contexte. |
...
| D8 | ✅ Résolue 2026-05-24 (Sprint Dettes Techniques) | `computeTelecomSummary.incompleteItems` comptait `status === 'needs_manual_completion' OR manual_completion_status !== 'complete'`. Le `OR` capturait les items `finance_synced` dont `manual_completion_status` n'était pas exactement `'complete'` (observé : 100 vs 94 sur Helios One). | **Résolu** : `||` → `&&` ligne 28 de `src/services/pm/telecomSummary.service.ts`. Sémantique correcte : un item est incomplet ssi `status === 'needs_manual_completion'` ET `manual_completion_status !== 'complete'`. Test smoke `backend/tests/pm/telecom-summary-d8.test.mjs` (4 assertions) valide AND vs régression OR, dont reproduction Helios-like 94/100. `tsc --noEmit` ✅. |
...
| D13 | ✅ Résolue 2026-05-25 (WorkItem en Sprint Dettes Techniques 2026-05-24 ; Milestone en Sprint DH12 2026-05-25) | Sous-tâches WorkItem absentes — pas de `parentId` self-relation. Périmètre Milestone reporté à un sprint dédié. | **Résolu (WorkItem)** : migration `20260524091509_add_workitem_parent` (parentId String? + self-relation `WorkItemChildren`, `onDelete: Restrict`, index parentId). Nouveau service `backend/services/pm/workItemHierarchy.service.mjs` : `createSubTask` (depth ≤ 3 garde), `listSubTasks`, `moveSubTask` (anti-cycle + anti-cross-project + depth recompute via maxSubtreeDepth), `rollupStatus` récursif en tx (all completed → complete, any blocked → at_risk, any in-progress → in-progress). `updateWorkItem` déclenche rollup quand status change. `getProjectById` eager-load 3 niveaux. 3 nouvelles routes : `POST/GET /api/v1/pm/work-items/:id/subtasks`, `PATCH /api/v1/pm/work-items/:id/parent` (assertPermission). Statut `at_risk` ajouté à WorkItemStatus union. Frontend : nesting + indent + toggle expand/collapse + badge X/Y + warning icon (vue liste). Section sous-tâches dans le drawer avec form inline + bouton désactivé+tooltip à depth 3. Tests `pm-subtasks.test.mjs` 8/8 ✓ (incl. propagation récursive, cycle, FK Restrict P2003, soft-delete preserves children). Bug détecté par les tests : `at_risk` n'était pas reclassifié comme `blocked` en remontée → ajouté à `BLOCKED_STATUSES`. 4 commits : `a701dfa`, `41b9f2a`, `2632cf1`, `1d527fa`. **Milestone parentId fermé en Sprint DH12 (2026-05-25)** — voir journal du jour. 4 commits : `0598027`, `2fa8936`, `90ac71e`, `7a40c77`. |
| D14 | ✅ Résolue 2026-05-22 (Sprint 7.6) | Payload 409 `MILESTONE_BLOCKED` sans `blockers[]` structuré. | **Résolu** : (1) Service `milestones.service.mjs:352-365` — construit `blockersList = [{ id, title }]` à partir du `findMany` existant, l'attache à `err.blockers` avant `throw`. (2) Route handler `projects.routes.mjs:236-239` — étendu pour forwarder `err.blockers` ET `err.field` dans le payload JSON. (3) Frontend `MilestonesPage.tsx` — nouveau state `deleteBlockers`, extraction depuis `ApiError.payload.blockers` si `code === 'MILESTONE_BLOCKED'`, transmis en prop au `DeleteMilestoneModal`. (4) Modal — `isBlocked = (blockers?.length ?? 0) > 0` (plus de message-sniffing), liste `<ul>` rendue avec icône `Link2` par dépendant. Smoke E2E : `POST A`, `POST B dependsOn A`, `DELETE A` → HTTP 409 + `{ error, code: 'MILESTONE_BLOCKED', blockers: [{ id, title }] }`. **D14 fermée.** |
| D15 | ✅ Résolue 2026-05-23 (Sprint HRM-1.4 — commits `574517b`, `cee6626`) | Synchronisation assignés télécom → HRM : lors d'un import bulk télécom (`importWorkItems`), les assignés présents dans le fichier qui n'existent pas dans `User`/HRM doivent être créés automatiquement comme **contractor** dans le module HRM avant d'être liés au projet. La liste des personnes assignables à un projet doit provenir exclusivement du HRM (pas de noms libres saisis depuis l'import). | **Résolu** : (1) helper `upsertContractor` côté backend (idempotent par email) ; (2) pre-pass dans `bulkImportTelecomWorkItems` matérialise un contractor HRM par `team` distinct avant `createWorkItem` — identité synthétisée depuis le team (slug email, firstName/lastName, `creationSource='TELECOM_IMPORT'`, `externalRef='team:<raw>'`) ; (3) endpoint `GET /api/v1/hrm/employees?assignable=true&projectId=...` alimentant les pickers ; (4) `WorkItemAssigneeSelect` typeahead créé (n'existait pas — `<input type=text>` libre auparavant), free text préservé pour les noms hors-référentiel. E2E vérifié : 2 rows même team → 1 contractor matérialisé en transaction, idempotent. Voir aussi : notes HRM-1.4 dans `docs/NEOX_HRM_PLAN.md`. |
