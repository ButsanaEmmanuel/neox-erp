...
| D10 | **🚨 CRITIQUE** — Découvert pendant setup preview Sprint 4 (2026-05-18) | `auth-server.mjs:236-240` : `verifyPassword` fait un fallback **plain-text equality** si le hash stocké ne contient pas `:` (séparateur scrypt salt/hash). Au moins un compte admin (`ebutsana@neox.io`, id `usr_ebutsana_full_access_20260321`) a son `passwordHash` stocké **en clair** en DB (valeur 16 chars sans `:`). **Compromission immédiate si la DB fuite** : credentials admin lisibles directement. Sécurité ≪ Sprint 3 attendrait sur l'API mais c'est plus grave. | **Sprint sécurité dédié, priorité absolue** : (1) re-hash tous les passwords stockés en clair via `hashPassword(plainText)`, (2) supprimer le fallback `plainText === storedPasswordHash` en l.240, (3) audit complet auth (token revocation, password policy, rate limiting login, etc.). À planifier hors module PM. |
