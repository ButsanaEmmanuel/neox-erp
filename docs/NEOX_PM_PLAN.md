...
| D10b | Sprint 4 (Hotfix D10) | Password admin `20153gcvkj5QAH8t` exposé dans l'historique git (commit `6c4d9a3` détaillant D10 + messages de session). Re-hash D10 ferme la voie DB→password, pas la voie git-history→password. | Rotation manuelle du password admin via UI ou script dédié, après commit D10. Pas un fix code. |
